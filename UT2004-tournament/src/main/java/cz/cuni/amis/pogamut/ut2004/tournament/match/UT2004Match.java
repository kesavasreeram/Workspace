		...
		//
		{
			...
			try {
				for (UT2004BotConfig botConfig : config.getBots().values()) {
					...
					// TEAM?
					if (botConfig.getTeamNumber() >= 0 && botConfig.getTeamNumber() < 4) {
						botConfig.getParameters().put(PogamutUT2004Property.POGAMUT_UT2004_BOT_INIT_TEAM.getKey(), botConfig.getTeamNumber());
					}
					
					// SKIN?
					if (botConfig.getSkin() != null) {
						botConfig.getParameters().put(PogamutUT2004Property.POGAMUT_UT2004_BOT_INIT_SKIN.getKey(), botConfig.getSkin());
					}
					
					// DESIRED SKILL LEVEL?
					if (botConfig.getDesiredSkill() >= 0) {
						botConfig.getParameters().put(PogamutUT2004Property.POGAMUT_UT2004_BOT_INIT_SKILL.getKey(), botConfig.getDesiredSkill());
					}					
		...
		//
		{
			...
			try {
				for (UT2004NativeBotConfig botConfig : config.getNativeBots().values()) {
					...
					// ADD NATIVE BOT
					AddBot addBotCommand = new AddBot().setSkill(botConfig.getDesiredSkill());
